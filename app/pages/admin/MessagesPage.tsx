    ...
    <PageContainer>
      <div className="space-y-5">
        <PageHeader
          ...
          description={language === 'en' ? 'Manage incoming messages from contact forms.' : 'إدارة الرسائل الواردة من نماذج الاتصال.'}
        />

        <div className="flex flex-col sm:flex-row gap-3">
          <div className="flex-1 flex items-center gap-2 px-4 py-2.5 bg-[var(--card)] border border-[var(--border)] rounded-xl">
            <Search className="w-4 h-4 text-[var(--text-secondary)]" />
            <input value={search} onChange={e => { setSearch(e.target.value); setPage(1); }} placeholder={language === 'en' ? 'Search messages...' : 'بحث في الرسائل...'} className="w-full bg-transparent text-sm outline-none text-[var(--foreground)]" />
          </div>
          ...
          <div className="lg:col-span-3 bg-[var(--card)] border border-[var(--border)] rounded-2xl overflow-hidden shadow-sm">
            {loading ? <div className="p-6 space-y-4">{[1,2,3,4,5].map(i => <div key={i} className="h-16 skeleton rounded-xl" />)}</div> : !data || data.messages.length === 0 ? (
              <div className="p-12 text-center">
                <Mail className="w-12 h-12 text-[var(--text-secondary)] mx-auto mb-3 opacity-50" />
                <p className="text-[var(--text-secondary)] text-sm">{language === 'en' ? 'No messages found.' : 'لا توجد رسائل.'}</p>
              </div>
            ) : (
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between gap-2">
                          <span className={`text-sm truncate ${m.status === 'unread' ? 'font-bold text-[var(--foreground)]' : 'font-medium text-[var(--foreground)]'}`}>{m.senderName}</span>
                          <span className="text-[10px] text-[var(--text-secondary)] shrink-0">
                            {new Date(m.createdAt).toLocaleDateString(language === 'ar' ? 'ar-EG' : 'en-US', { month: 'short', day: 'numeric' })}
                          </span>
                        </div>
            ...
            {selected ? (
              <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="bg-[var(--card)] border border-[var(--border)] rounded-2xl p-6 shadow-sm sticky top-24">
                <div className="flex items-start justify-between mb-4">
                  <div>
                    <h3 className="font-bold text-[var(--foreground)]">{selected.senderName}</h3>
                    <p className="text-xs text-[var(--text-secondary)] mt-0.5">{selected.senderEmail} · {selected.senderPhone}</p>
                  </div>
                  <div className="flex gap-1">
                    {selected.status === 'unread' ? 
                      <button onClick={() => handleRead(selected)} className="p-1.5 hover:bg-[var(--secondary)] rounded-lg text-[var(--text-secondary)]" title="Mark read">
                        <MailOpen className="w-4 h-4" />
                      </button> : 
                      <button onClick={() => handleUnread(selected)} className="p-1.5 hover:bg-[var(--secondary)] rounded-lg text-[var(--text-secondary)]" title="Mark unread">
                        <Mail className="w-4 h-4" />
                      </button>
                    }
                    <button onClick={() => handleDelete(selected.id)} className="p-1.5 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg text-red-500">
                ...
                {selected.reply && (
                  <div className="mb-5">
                    <p className="text-xs font-semibold text-[var(--text-secondary)] mb-2 flex items-center gap-1">
                      <Reply className="w-3.5 h-3.5" /> {language === 'en' ? 'Your reply' : 'ردك'}
                    </p>
                    <div className="bg-green-50 dark:bg-green-900/10 p-4 rounded-xl text-sm text-green-800 dark:text-green-300 leading-relaxed whitespace-pre-wrap">{selected.reply}</div>
                  </div>
                )}
