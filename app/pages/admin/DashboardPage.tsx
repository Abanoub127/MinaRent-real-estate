...
export const DashboardPage: React.FC = () => {
  ...
  return (
    <motion.div variants={cV} initial="hidden" animate="visible" className="space-y-6 p-4 md:p-6 lg:p-8">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
        {statCards.map((s,i)=>(<motion.div key={i} variants={iV} className={`group p-4 sm:p-6 rounded-xl border border-[var(--border)] shadow-sm relative overflow-hidden transition-all duration-300 hover:-translate-y-[2px] hover:shadow-md ${i===0?'bg-gradient-to-br from-[#1B2B4B] to-[#0D1B2A] text-white':'bg-gradient-to-br from-[var(--card)] to-[var(--secondary)]'}`}>
          ...
          <div className="relative z-10"><h3 className={`text-[0.65rem] sm:text-xs font-medium mb-1 ${i===0?'text-white/80':'text-[var(--text-secondary)]'}`}>{s.title}</h3><p className={`text-xl sm:text-2xl font-bold tracking-tight ${i===0?'text-white':'text-[var(--foreground)]'}`}>{s.value}</p></div>
        </motion.div>))}
      </div>

      <div className="grid lg:grid-cols-3 gap-6">
        <motion.div variants={iV} className="lg:col-span-2 bg-[var(--card)] p-6 rounded-xl border border-[var(--border)] shadow-sm">
          <h3 className="text-lg font-bold text-[var(--foreground)] mb-4">{t('dashboard.revenueChart')}</h3>
          <div className="h-[200px] lg:h-[280px] w-full"><ResponsiveContainer width="100%" height="100%"><AreaChart data={stats.monthlyRevenueData||[]} margin={{top:10,right:10,left:0,bottom:0}}>
        ...
        <motion.div variants={iV} className="bg-[var(--card)] p-6 rounded-xl border border-[var(--border)] shadow-sm flex flex-col">
